# input-number-single-array-form

`返回[1]数组格式化值，默认初始化onchage值[1]`
...
import { InputNumberSingleArrayForm } from 'aem-ui';
...
const Demo: React.FC = () => {
  ...
  return (
    <>
      <InputNumberSingleArrayForm value={value} onChange={onChange} />
    </>
  );
};
