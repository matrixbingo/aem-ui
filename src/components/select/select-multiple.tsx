...
const SelectMultiple = <T extends number|string>(props: SelectMultipleProps<T>) => {
  const { value: initValue, onChange, defaultOption, options, style, placeholder, ...restProps } = props;
  ...
  const allIds = useMemo(
    () => {
      return DataUtil.toArrByPath(options, 'id');
    },
    [options],
  );
  ...
  const onChangeCallBack = (ids) => {
    ...
    if (isEqual(allIds, ids) || allIds?.length === ids?.length) { // TODO isEqual
      setChecked(true);
    } else {
      setChecked(false);
    }
      ...
      dropdownRender={(allSelectValue) => (
        <div>
          <div style={{ padding: '4px 8px 8px 8px', cursor: 'pointer' }}>
            <Checkbox checked={checked} onChange={onChangeCheckbox}>全选</Checkbox>
          </div>
